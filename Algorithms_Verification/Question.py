class Question:
  ...
  def check_user_input(user_input_raw, answer, nombre):
    user_input = []
    for i in range(len(user_input_raw)):
      user_input.append( user_input_raw[i].get() )

    if nombre == "Scale" or nombre == "Chord":
      user_input = Question.convertToInt(user_input)
    else:
